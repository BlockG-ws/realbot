from adapters.db.models import FediTokens, FediSecrets
...
async def fedi_instance_is_misskey(instance_domain: str) -> bool | None:
    ...
    is_misskey = await FediSecrets.get_or_none(instance_domain=instance_domain).values("is_misskey")
    return is_misskey[0] if is_misskey else None

async def get_fedi_client_info(instance_domain: str) -> dict:
    """Retrieve Fediverse instance client info by instance_domain."""
    client_id, client_secret = await FediSecrets.get_or_none(instance_domain=instance_domain).values("client_id", "client_secret")
    return {
        "client_id": client_id,
...
async def get_fedi_user_instance_domains(user_id: int) -> list[str]:
    ...
    tokens = await FediTokens.filter(user_id=user_id).values_list("instance_domain", flat=True)
    return list(tokens)
...
async def get_fedi_user_cred(instance_domain: str, user_id: int) -> str:
    ...
    token = await FediTokens.get_or_none(instance_domain=instance_domain, user_id=user_id)
...
async def update_fedi_user_cred(instance_domain: str, user_id: int, access_token: str) -> None:
    ...
    token_obj, _ = await FediTokens.get_or_create(instance_domain=instance_domain, user_id=user_id)
...
async def update_fedi_client_info(instance_domain: str, is_misskey: bool,client_id: str, client_secret: str) -> None:
    ...
    obj, _ = await FediSecrets.get_or_create(
